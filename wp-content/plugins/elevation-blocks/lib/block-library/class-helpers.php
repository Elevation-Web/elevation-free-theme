 ...
 * Class Helpers
 *
 * @package ElevationBlocks\Lib\BlockLibrary
 */
class Helpers {
	...
	public static function global_image( $image, $args = null ) {
		...
		if ( $is_url ) {
			...
			$img = sprintf( '<img src="%s" alt="%s" class="%s" loading="%s" decoding="async">', esc_url( $image ), isset( $image['alt'] ) && $image_args['alt'] ? esc_attr( $image_args['alt'] ) : esc_attr( '' ), esc_attr( $image_args['class'] ), esc_attr( $image_args['loading'] ) );
		} else {
	...
	public function defer_scripts( $tag, $handle, $source ) {
		if ( ! is_admin() && ( str_contains( $source, '/build/blocks/' ) || str_contains( $source, '/build/assets/' ) ) ) {
			return;
		}
