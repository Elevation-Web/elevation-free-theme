...
const Edit = (props) => {
	...
	const buttonOptionsArray = Object.values(buttonOptions.buttonOptions).map(
		(entry) => {
			return {
				label: __(entry.label, 'elevation-free-blocks'),
				value: entry.value,
			};
		}
	);
	...
	const controls = (
		<InspectorControls>
			<PanelBody title={__('Button Styles', 'elevation-free-blocks')}>
				<SelectControl
					label={__('Button Style', 'elevation-free-blocks')}
					value={btnStyle}
			...
			<PanelBody title={__('Button Link', 'elevation-free-blocks')}>
				<>
					{url && (
						<a
							href={url}
			...
			<PanelBody title={__('Button Aria Label', 'elevation-free-blocks')}>
				<__experimentalInputControl
					value={ariaLabel}
	...
	return (
		<>
			...
			<div {...blockProps}>
				<TextControl
					placeholder={__('Button text...', 'elevation-free-blocks')}
					value={text}
					onChange={(value) =>
