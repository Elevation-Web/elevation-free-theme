...
const cards = document.querySelectorAll(`.team-bios-default .card button`);
var currentPopup;
...
cards.forEach((card, idx) => {
	card.addEventListener('click', (e) => {
		...
		Swal.fire({
			...
			customClass: {
				container: `team-bios-default__popup-container ${id} `,
			},
			showClass: { popup: 'team-bios-default-show' },
		});
...
const buttonFilters = document.querySelectorAll(
	'.team-bios-default-container__filters__list__item__button'
);
...
if (buttonFilters?.length > 0) {
	buttonFilters.forEach((buttonFilter) => {
		buttonFilter.addEventListener('click', (e) => {
			...
			const cards = document.querySelectorAll('.team-bios-default .card');
