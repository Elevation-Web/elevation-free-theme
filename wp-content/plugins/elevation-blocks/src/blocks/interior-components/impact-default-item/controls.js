...
export const Controls = (props) => {
	...
	return (
		<InspectorControls>
			<PanelBody title={__('Impact Settings', 'elevation-blocks')}>
				<TextControl
					label={__('Value', 'elevation-blocks')}
					value={value}
					onChange={(newValue) => setAttributes({ value: newValue })}
				/>
				<ToggleControl
					label={__('Remove Comma', 'elevation-blocks')}
				...
				/>
				<SelectControl
					label={__('Title Size', 'elevation-blocks')}
					value={title_size}
					options={[
						{ label: 'h1', value: 'h-1' },
						{ label: 'h2', value: 'h-2' },
