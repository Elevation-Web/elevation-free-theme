...
const globalAnimations = () => {
  const baseSelectors = ['#primary p', '#primary img', '#primary .cta', '#primary li', '#primary h1', '#primary h2', '#primary h3', '#primary h4', '#primary h5', '#primary h6', '.fade-in-bottom', '.timeline-vertical-item'];
  const exclusionSelectors = ['.accordion-item__item p', '.banner-full-img img', '.video-full-screen-pop-up img', '.accordion-item__item img', '.accordion-item__item .cta', '.team-bios-with-pop-up .title', '.team-bios-with-pop-up img', '.team-bios-with-pop-up p', '.team-bios-with-pop-up .cta', '.no-animate *', '.no-animate', '.logos-descriptions-item img', '.logos-carousel-item img', '.testimonial-carousel-item__image img', '.testimonial-single-item__image img', '.timeline-vertical-item *', '.timeline-horizontal-item img', '.team-bios-modal li'];
...
const loadAssetsCSSandJS = () => {
  ...
  const path = `${_utils__WEBPACK_IMPORTED_MODULE_0__.base}/build/assets`;
  // Load CSS file
...
const loadBlocksCSSandJS = (firstsBlocks = [], exclude = false, addIfExist = [], excludedBlocks) => {
  ...
  if (addIfExist.length > 0) {
    ...
    const validAddIfExist = addIfExist.filter(id => blocksInDOM.includes(id));
  ...
  uniqueBlocksIds?.forEach(function (id) {
    const path = `${_utils__WEBPACK_IMPORTED_MODULE_0__.base}/build/blocks/${id}`;
    // Load CSS file
    (0,_utils__WEBPACK_IMPORTED_MODULE_0__.addCSS)(path);
...
const firstBlocks = ['interior-components/spacer', 'interior-components/swiper', 'interior-components/group'];
...
(0,_scripts_global_animations__WEBPACK_IMPORTED_MODULE_4__.globalAnimations)();

// Add more blocks to the firstBlocks array if needed
// firstBlocks.push('BLOCK_ID_1');

(0,_scripts_loadBlocksCSSandJS__WEBPACK_IMPORTED_MODULE_0__["default"])(firstBlocks, true, addIfExist, excludedBlocks);
