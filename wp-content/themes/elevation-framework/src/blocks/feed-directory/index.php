...
/**
 * Feed Directory Block Callback Function.
 *
 ...
 */
function feed_directory_block_render_callback($block, $content = '', $is_preview = false, $post_id = 0, $wp_block = false, $context = false)
{
														...
														<?php else : ?>
															...
															<div class="card__body">
																<?php if (isset($item['title']) && !empty($item['title'])) : ?>
																	<h2 class="card__title line-clamp-2 h6">
																		<span>
																			<?= esc_html($item['title']); ?>
																		</span>
																	</h2>
															...
															<?php if (isset($item['link']) && !empty($item['link'])) : ?>
																...
																<a href="<?= esc_url($item['link']['url']); ?>" target="<?= esc_attr($target); ?>" class="stretched-link stretched-link-custom">Read More <?= esc_html($item['title']); ?></a>
															<?php endif; ?>
												...
												<?php if ($related_layout == "grid") : ?>
													<div class="col">
														...
														<?php else : ?>
															...
															<?php Helpers::global_image($image, ['size' => 'medium', 'class' => 'card__image card__image--hover-effect' . $card_image_change, 'alt' => $alt_image]); ?>
									...
									<?php elseif ($related_options == 'by_posts') : ?>
										...
										if ($featured_posts) :
											...
											foreach ($featured_posts as $featured_post) :
												...
												$categories = get_the_terms($featured_post->ID, 'resources_categories');
												...
												$downloadable = get_field('downloadable', $featured_post->ID);
												if (isset($downloadable) && !empty($downloadable)) {
													$card_image_change = ' card__image--pdf';
												}
												$video_youtube = get_field('youtube', $featured_post->ID);
												if (isset($video_youtube) && !empty($video_youtube)) {
													$card_image_change = ' card__image--video';
												}
										?>
