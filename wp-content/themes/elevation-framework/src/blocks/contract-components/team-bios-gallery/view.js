...
const cards = document.querySelectorAll(`.team-bios-gallery .card button`);
var currentPopup;
...
cards.forEach((card, idx) => {
	card.addEventListener('click', (e) => {
		...
		Swal.fire({
			...
			customClass: {
				container: `team-bios-gallery__popup-container ${id} `,
			},
			showClass: { popup: 'team-bios-gallery-show' },
		});
...
const buttonFilters = document.querySelectorAll(
	'.team-bios-gallery-container__filters__list__item__button'
);
...
if (buttonFilters?.length > 0) {
	buttonFilters.forEach((buttonFilter) => {
		buttonFilter.addEventListener('click', (e) => {
			...
			const cards = document.querySelectorAll('.team-bios-gallery .card');
