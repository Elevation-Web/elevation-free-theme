...
final class Load {
	...
	public function autoload( $class_to_load ) {
		...
		if ( ! class_exists( $class_to_load ) ) {
			...
			if ( is_readable( $class_file ) ) {
				...
				$warning_message = sprintf( esc_html__( '"Can\'t find %1$s" for "%2$s" in "%3$s".', 'elevation-blocks' ), $class_file, $class_to_load, __NAMESPACE__ );
	...
	public static function class_file( $class_name ) {
		...
		if ( strpos( $filename, 'build' ) !== false ) {
			return ELEVATION_BLOCKS_DIR . $filename . '.php';
		}
		return ELEVATION_BLOCKS_DIR . 'lib/' . $filename . '.php';
	}
