...
class Notices {
	...
	public function __construct() {
		...
		if ( $active_theme->get( 'Name' ) !== 'Elevation Free' ) {
			add_action( 'admin_notices', array( $this, 'elevation_blocks_check_active_theme' ) );
		}
	}
	 ...
	 */
	public function elevation_blocks_check_active_theme() {
		$class          = 'notice notice-error is-dismissible';
		$message_part_1 = __( 'Elevation Blocks ', 'elevation-blocks' );
		$message_part_2 = __( 'requires the ', 'elevation-blocks' );
		$url            = home_url() . '/wp-admin/theme-install.php?search=Elevation%20Free';
		$message_part_3 = __( 'Elevation Free Theme', 'elevation-blocks' );
		$message_part_4 = __( '  to be activated for full functionality.', 'elevation-blocks' );
